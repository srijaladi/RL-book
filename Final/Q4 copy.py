...
def ftr_func_1(x, C = 100, z = 0.1, gamma = user_gamma):
    state = x[0].state
    action  = x[1]
...
def ftr_func(x, C = 100, z = 0.1, gamma = user_gamma):
    ...
    if action == 1:
        if state.quality == 0:
            return [0,0,0]
        return [state.quality, gamma, 0]
    else:
        return [0,z * gamma, z * gamma * (C - state.quality)]
...
ffs = [(lambda x, i=i: all_ftrs[x[0].state.quality][x[1]][i]) for i in range(3)]
...
for episode in range(numEpisodes):
    ...
    for s in cropMDP.non_terminal_states:
        for a in [0,1]:
            diff += abs(last_q_func_approx((s,a)) - q_func_approx((s,a)))
    if diff < 10:
        print("diff")
        break
    ...
    check: bool = True
    for s in cropMDP.non_terminal_states:
        correctAction = opt_policy_vi.act(s).sample()
        wrongAction = abs(correctAction - 1)
        if (q_func_approx((s,correctAction)) > q_func_approx((s,wrongAction))):
            continue
        check = False
        break
    if check and episode > 10:
        break
    """
