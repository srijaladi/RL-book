...
class WindyGrid:
    ...
    def get_transition_probabilities(self, nt_state: Cell) \
        ...
        allMoves: Sequence[Move] = [(0,1), (1,0), (0,-1), (1,0)]
        nonTerminalStates = self.get_all_nt_states()
        
        allMoveResults: Set[Tuple[Move, Cell]] = self.get_actions_and_next_states(nt_state)
        
        for possMove, newState in allMoveResults:
            #newState = (nt_state[0] + possMove[0], nt_state[1] + possMove[1])
            #print("old state: " + str(nt_state) + " and newState: " + str(newState))
            
            if self.is_valid_state(newState) == False:
                continue
            
            if newState in self.terminals:
                move_d = {(newState, -1): 1}
                d[possMove] = Categorical(move_d)
                #print(move_d)
                continue
            ...
            newStateUp = (newRow+1, newCol)
            newStateDown = (newRow-1, newCol)
            
            bumpProb = 0
            upProb = 0
            downProb = 0
            
            P1 = self.wind[newCol][0]
            P2 = self.wind[newCol][1]
            constantProb = 1 - P1 - P2
            
            if self.is_valid_state(newStateUp) == False:
                bumpProb += P2 #P2
            else:
                upProb += P2
                
            if self.is_valid_state(newStateDown) == False:
                bumpProb += P1 #P1
            else:
                downProb += P1
            
            move_d: Dict[Tuple[Cell, float], float] = {}
            
            bumpedTuple = (newState, -1 - self.bump_cost)
            upTuple = (newStateUp, -1)
            downTuple = (newStateDown, -1)
            constTuple = (newState, -1)
            
            move_d[bumpedTuple] = bumpProb
            move_d[downTuple] = downProb
            move_d[upTuple] = upProb
            move_d[constTuple] = constantProb
    ...
    def get_sarsa_vf_and_policy(
        ...
        for episode_num in range(episodes):
            ...
            while not(currState in self.terminals):
                currAction: Move = self.epsilon_greedy_action(currState, q, epsilon)
                transitionStep: Tuple[Cell, float] = sample_func(currState, currAction)
                ...
                if nextState in self.terminals:
                    ...
                    nextAction = self.epsilon_greedy_action(nextState, q, epsilon)
                    qNextVal = q[nextState][nextAction]
                    
                new_predict = currReward + qNextVal
                diff = new_predict - q[currState][currAction]
                #step_size = 1
                change = diff * step_size
                
                q[currState][currAction] += change
                
                currState = nextState
             
            """
            transitionStep: Tuple[Cell, float] = sample_func(state, currAction)
    ...
    def get_q_learning_vf_and_policy(
        ...
        for episode_num in range(episodes):
            ...
            while not(currState in self.terminals):
                currAction: Move = max(q[currState], key = q[currState].get)
                transitionStep: Tuple[Cell, float] = sample_func(currState, currAction)
                ...
                if nextState in self.terminals:
                    ...
                    nextAction = max(q[nextState], key = q[nextState].get)
                    qNextVal = q[nextState][nextAction]
